...
from jace.computer.security import ComputerPathError, relative_display
...
def _normalise_workspace_hint(value: str) -> str:
    return " ".join(
        part
        for part in re.sub(
            r"[^a-z0-9]+",
...
def _workspace_matches_message(workspace, message: str) -> bool:
    text = _normalise_workspace_hint(message)
    if not text:
        return False

    label = _normalise_workspace_hint(str(workspace.label or ""))
    root_name = _normalise_workspace_hint(Path(workspace.root_path).name)
    ...
    if root_name and len(root_name) >= 3:
        return bool(
            re.search(
                rf"(?:^| )(?:project |repo |repository |workspace )?{re.escape(root_name)}(?: |$)",
                text,
            )
        )
...
async def _intelligence_workspace(
    context: ToolContext,
    workspace_id: str | None,
):
    if workspace_id:
        return await _workspace_for_tool(
            context,
            workspace_id,
        )
    ...
    if not readable:
        raise ToolError(
            "No active readable computer workspace is configured. "
            "Add/enable one in the Computer settings first."
        )
    ...
    choices = ", ".join(
        f"{workspace.label} [{workspace.id}]"
        for workspace in readable
    )

    if len(message_matches) > 1:
        raise ToolError(
            "More than one active workspace matched the request. "
            f"Specify one workspace_id. Candidates: {choices}."
        )

    raise ToolError(
        "More than one active readable workspace is configured and the request "
        "did not identify one uniquely. Specify workspace_id. "
...
class WorkspaceOverviewInput(BaseModel):
    workspace_id: str | None = Field(
        ...
        max_length=36,
        description=(
            "Optional approved workspace ID. Omit it when there is one active "
            "readable workspace or the user's wording uniquely names one."
        ),
    )
    max_files: int = Field(
        default=5000,
    ...
    definitions = [
        ToolDefinition(
            ...
            description=(
                ...
                "workspace. workspace_id is optional when a single active "
                "readable workspace exists or the user's wording uniquely names "
                "one. Includes project/framework markers, package managers, Git "
        ...
        ToolDefinition(
            ...
            description=(
                ...
                "workspace. workspace_id can be omitted when the workspace is "
                "unambiguous. Dependency/build/cache directories and protected "
        ...
        ToolDefinition(
            ...
            description=(
                ...
                "workspace_id can be omitted when the workspace is unambiguous."
            ),
            category="Filesystem",
            risk="read",
            default_permission="ask",
        ...
        ToolDefinition(
            ...
            description=(
                ...
                "workspace text file without changing it. workspace_id can be "
                "omitted when the workspace is unambiguous. Returns the current "
