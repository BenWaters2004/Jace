...
from jace.connections.service import (
    ...
    developer_oauth_client_id,
    developer_oauth_tenant,
...
async def _client_config(session: AsyncSession, provider: ProviderDefinition):
    ...
    client_id = developer_oauth_client_id(provider.id) or (row.client_id.strip() if row and row.client_id.strip() else None)
    if not client_id:
        raise ValueError(f"{provider.name} is not configured for this Jace installation. Set JACE_{provider.id.upper()}_CLIENT_ID in backend/.env and restart Jace.")
    config = _oauth_config_dict(row)
    if provider.id == "microsoft" and developer_oauth_client_id(provider.id):
        config["tenant"] = developer_oauth_tenant(provider.id) or "common"
    class EffectiveOAuthConfig:
        def __init__(self, client_id: str, config: dict):
            self.client_id = client_id
            self.config_json = json.dumps(config, separators=(",", ":"))
    return EffectiveOAuthConfig(client_id, config)
