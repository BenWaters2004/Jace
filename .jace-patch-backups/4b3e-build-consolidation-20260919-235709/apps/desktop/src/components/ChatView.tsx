...
// JACE_STEP3_RICH_CHAT_PHASE3E
// JACE_STEP3_RICH_CHAT_PHASE3E1
// JACE_STEP3_RICH_CHAT_PHASE3E2
// JACE_STEP3_RICH_CHAT_PHASE3F
export function ChatView(props: ChatViewProps) {
  ...
  function addComposerFiles(files: File[]) {
    if (!props.online || props.isGenerating || files.length === 0) return;

    const remaining = Math.max(
      0,
      props.attachmentLimit - props.pendingAttachments.length,
    );
    if (remaining === 0) return;

    props.onFilesAdded(files.slice(0, remaining));
  }
  ...
  function dragEnterFiles(event: DragEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!Array.from(event.dataTransfer.types).includes("Files")) return;
    if (!props.online || props.isGenerating) return;

    composerDragDepthRef.current += 1;
    setComposerDragActive(true);
  }

  function dragOverFiles(event: DragEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!Array.from(event.dataTransfer.types).includes("Files")) return;
    event.dataTransfer.dropEffect = "copy";
  }

  function dragLeaveFiles(event: DragEvent<HTMLFormElement>) {
    event.preventDefault();
  ...
  return (
    <section className="content-shell chat-shell">
                ...
                >
                  <div className="message-body">
                    ...
                    <MessageFrame
                      ...
                      agentHandoff={Boolean(message.model?.startsWith("agent:"))}
                    
                      workDurationMs={message.stats?.timeToFirstTokenMs ?? message.stats?.totalDurationMs ?? null}
                      onRetry={
                        !props.isGenerating &&
                        message.role === "assistant" &&
                        !message.model?.startsWith("agent:") &&
                        !message.isStreaming &&
                        displayedMessages[displayedMessages.length - 1]?.id === message.id
      ...
      <div className="composer-wrap">
            ...
            {props.pendingAttachments.map((attachment) => (
              <div className="pending-attachment" key={attachment.id}>
                ...
                <button
                  ...
                  <svg
                    className="pending-attachment-remove-icon"
                    viewBox="0 0 20 20"
                    aria-hidden="true"
                    focusable="false"
                  >
        ...
        <div
          className="chat-sr-only chat-a11y-status"
          role="status"
          aria-live="polite"
          aria-atomic="true"
        >
          {props.isGenerating ? `${props.assistantName} is responding.` : ""}
        </div>
        <form
          className="composer multimodal-composer"
          onSubmit={props.onSubmit}
          onDragEnter={dragEnterFiles}
          ...
          <button
            ...
            aria-label="Attach files"><span className="composer-plus-glyph" aria-hidden="true">
            <svg
              className="composer-plus-icon"
              viewBox="0 0 20 20"
              focusable="false"
            >
          ...
          <textarea
            ...
            onPaste={pasteFiles}
            aria-label={`Message ${props.assistantName}`}
            value={props.input}
            ...
            aria-describedby="chat-composer-hint"/>
                    ...
                    {props.isGenerating ? (
                      <button
                        type="button"
                        className="stop-button composer-round-action"
                    ...
                    ) : (props.input.trim() || props.pendingAttachments.length > 0) ? (
                      <button
                        className="send-button composer-round-action"
                        type="submit"
                        disabled={!props.online || !props.selectedModel}
                        title="Send message"
                        aria-label="Send message"
                      >
                        <svg viewBox="0 0 20 20" aria-hidden="true">
                          <path d="M10 15V5" />
                          <path d="m5.8 9.2 4.2-4.3 4.2 4.3" />
                        </svg>
                      </button>
                    ) : (
                      <button
                        className="composer-talk-button composer-round-action"
                        type="button"
                        disabled={!props.online || !props.selectedModel}
                        title="Hold to talk"
                      ...
                      >
                        <svg viewBox="0 0 20 20" aria-hidden="true">
                          ...
                          <path d="M4.8 9.5a5.2 5.2 0 0 0 10.4 0" />
                          <path d="M10 14.7V18" />
                          <path d="M7.5 18h5" />
                        </svg>
                      </button>
        ...
        <div className="composer-hint"
          ...
          <span className="composer-hint-files">
            {props.pendingAttachments.length > 0
              ? `${props.pendingAttachments.length}/${props.attachmentLimit} attached · `
              : ""}
            Paste or drop files
          </span>
        </div>
