...
async def _mark_workflow_recovered(workflow_id: str) -> None:
    """Persist restart recovery on every child so the office can reconstruct it."""
    async with SessionLocal() as session:
        rows = await list_tasks(session, limit=500)
        changed = False
        for row in rows:
            ...
            if metadata.get("director_workflow_id") != workflow_id:
                continue
            if metadata.get("director_recovered") is True:
                continue
            metadata["director_recovered"] = True
            row.metadata_json = json.dumps(metadata, ensure_ascii=False)
            await append_event(
                session,
                row,
                event_type="director_recovered",
                message="Agent Director reconstructed this workflow after Jace restarted.",
...
async def resume_director_workflows() -> int:
    ...
    for workflow_id, (objective, conversation_id, mode, plan) in candidates.items():
        ...
        _track_workflow(task, workflow_id)
        await _mark_workflow_recovered(workflow_id)
        await runtime_events.publish(
            "agent.director.recovered",
            workflow_id=workflow_id,
            conversation_id=conversation_id,
        )
        resumed += 1
