...
export function PixelAgentOffice(props: {
  ...
  const engineRef =
    useRef<JacePixelOfficeEngine | null>(null);

  const workersRef =
    useRef(props.workers);
  const selectTaskRef =
    useRef(props.onSelectTask);

  useEffect(() => {
    workersRef.current = props.workers;
    engineRef.current?.syncWorkers(
      props.workers,
    );
  }, [props.workers]);
  ...
  useEffect(() => {
    ...
    const engine =
      new JacePixelOfficeEngine(canvas);
