...
export function JaceCore(props: {
  ...
  return (
    <section className={`cc-panel jace-core-panel state-${props.state}`}>
      ...
      <div className={`jace-core-stage mode-${props.state}`} style={stageStyle} aria-label={`${props.name} ${LABELS[props.state]}`}>
            ...
            {showWave ? (
              <div className={`core-waveform wave-${props.state}`} aria-hidden="true">
                {waveformBars.map((index) => {
                  ...
                  const liveHeight = props.state === "speaking" || props.state === "listening"
                    ? 8 + amplitude * (20 + centreBias * 23)
                    : 10 + centreBias * 12;
                  return (
                    <span
                      key={`wave-${index}`}
                      style={{
                        ...
                        height: `${Math.max(8, liveHeight)}px`,
                        opacity: 0.62 + amplitude * 0.38,
                      }}
                    />
                  );
                })}
