...
export function CalendarWorkspace() {
  ...
  const refreshAll = useCallback(
    async () => {
      ...
      try {
        ...
        if (
          syncResult.needs_reconnect
        ) {
          setError(
            "Calendar access needs authorization. "
            + "Reconnect the affected account in Settings → Connections.",
          );
        } else if (
          syncResult.errors.length > 0
        ) {
          setError(
            `Calendar sync completed with warnings: ${
  ...
  useEffect(
    () => {
      void refreshAll();
      // Initial calendar boot only; range changes load from the local cache.
        ...
        .filter(
          (event) => {
            if (
              ...
              return event.end_date_exclusive
                ? new Date(
                  `${event.end_date_exclusive}T12:00:00`,
                ) >= startOfDay(
                  now,
                )
                : false;
  ...
  function openCreate(
    ...
    if (!source) {
      setError(
        "No writable Jace calendar is available.",
      );
      return;
    }
  ...
  async function removeEvent(
    ...
    if (
      !window.confirm(
        `Delete "${event.title}" from the Jace calendar?`,
      )
    ) {
      return;
    }
  ...
  async function toggleSource(
    ...
    if (
      ...
      try {
        const updated = await updateCalendarSource(
          source.id,
          {
            enabled:
              nextEnabled,
        ...
        setError(
          ...
          instanceof Error
            ? sourceError.message
            : "Could not update the calendar source.",
        );
      }
    }
  }

  async function changeSourceColor(
    source: CalendarSource,
    color: string,
  ) {
    try {
      ...
      setError(
        ...
        instanceof Error
          ? sourceError.message
          : "Could not update the calendar colour.",
      );
    }
  }
                ...
                sources.map(
                  (source) => (
                    ...
                    >
                      ...
                      <input
                        type="color"
                        className="calendar-source-color-picker"
                        value={
                          validHexColor(
                            source.color,
                          )
                        ...
                        aria-label={
                          `Colour for ${source.name}`
                        }
                        title={
                          `Change colour for ${source.name}`
                        }
                        onChange={
                          (event) => (
                            void changeSourceColor(
                              source,
                              event.target.value,
                            )
                          )
...

                      {
                        ...
                        && (
                          <span
                            className="calendar-source-warning"
