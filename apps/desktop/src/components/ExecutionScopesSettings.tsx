...
interface ExecutionScopeDevice {
  id: string;
  name: string;
...
function deviceIsOnline(device: ExecutionScopeDevice | null): boolean {
  if (!device) return false;
  if (typeof device.connected === "boolean") return device.connected;
  return device.state === "online";
}

function deviceShells(device: ExecutionScopeDevice | null): ShellName[] {
...
export function ExecutionScopesSettings() {
  const [devices, setDevices] = useState<ExecutionScopeDevice[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setDevices(Array.from(deviceResponse.devices));
  ...
  const onlineDeviceCount = devices.filter((device) => deviceIsOnline(device)).length;
        ...
        >
              ...
              {devices
                ...
                .map((device) => (
                  <option key={device.id} value={device.id}>
                    {device.name} · {deviceIsOnline(device) ? "online" : "offline"}
                  </option>
                ))}
            </select>
          </label>
          ...
          {selectedDevice && (
            <div className="execution-scope-device-note">
              <span>
                {selectedDevice.platform ?? "unknown"} · {selectedDevice.architecture ?? "unknown"}
              </span>
              <span>
                process.runtime{" "}
