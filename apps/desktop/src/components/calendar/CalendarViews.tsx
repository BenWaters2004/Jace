...
function EventChip(props: {
  ...
  return (
    <button
      ...
      style={{ "--calendar-event-color": props.event.calendar_color } as CSSProperties}
      onClick={(event) => {
    ...
    >
      <span className="calendar-event-time">
        {eventTimeLabel(props.event, props.timezone)}
      </span>
...
function weekAllDaySegments(
  ...
  const weekEnd = addDays(weekStart, visibleDays);
  const allDay = events
    .filter((event) => {
      if (!event.all_day || !event.start_date || !event.end_date_exclusive) return false;
      return event.start_date < dateKey(weekEnd)
        && event.end_date_exclusive > dateKey(weekStart);
    })
    .slice()
    .sort((left, right) => {
      const startCompare = (left.start_date ?? "").localeCompare(right.start_date ?? "");
      if (startCompare !== 0) return startCompare;
      return (right.end_date_exclusive ?? "").localeCompare(left.end_date_exclusive ?? "");
    });
  ...
  allDay.forEach((event) => {
    const actualStart = parseDateKey(event.start_date!);
    const actualEnd = parseDateKey(event.end_date_exclusive!);
    const segmentStart = actualStart < weekStart ? weekStart : actualStart;
    const segmentEnd = actualEnd > weekEnd ? weekEnd : actualEnd;

    const column = Math.max(0, daysBetween(weekStart, segmentStart));
    const span = Math.max(1, daysBetween(segmentStart, segmentEnd));
    const interval = { start: column, end: column + span };
    ...
    segments.push({
      ...
      span,
      lane,
      startsHere: segmentStart.getTime() === actualStart.getTime(),
      endsHere: segmentEnd.getTime() === actualEnd.getTime(),
    });
  });
  ...
  return (
    <button
      ...
      className={[
        ...
        segment.startsHere ? "starts-here" : "continues-left",
        segment.endsHere ? "ends-here" : "continues-right",
      ].join(" ")}
      style={{
        gridColumn: `${segment.column + 1} / span ${segment.span}`,
        gridRow: `${segment.lane + 1}`,
        "--calendar-event-color": segment.event.calendar_color,
      } as CSSProperties}
      title={segment.event.title}
      onClick={(event) => {
        event.stopPropagation();
        props.onClick(segment.event);
      }}
    >
      {!segment.startsHere && <span className="calendar-continuation-mark">‹</span>}
      <span>{segment.event.title}</span>
      {!segment.endsHere && <span className="calendar-continuation-mark">›</span>}
    </button>
  );
}
...
export function MonthView(props: {
  ...
  const monthStart = startOfMonthGrid(props.cursor);
  const today = dateKey(new Date());

  const weeks = useMemo(
    () => Array.from({ length: 6 }, (_, weekIndex) => {
      const weekStart = addDays(monthStart, weekIndex * 7);
      const days = Array.from({ length: 7 }, (_, dayIndex) => addDays(weekStart, dayIndex));
      const multi = weekAllDaySegments(props.events, weekStart, 7);

      return {
        weekStart,
        days,
        segments: multi.segments,
        laneCount: multi.laneCount,
      };
    }),
    [monthStart.getTime(), props.events],
  );
  ...
  const timedByDate = useMemo(() => {
    ...
    props.events
      ...
      .forEach((event) => {
        const key = eventDateKey(event, props.timezone);
        if (!key) return;
        result.set(key, [...(result.get(key) ?? []), event]);
      });

    return result;
  }, [props.events, props.timezone]);
      ...
      <div className="calendar-month-weeks">
        {weeks.map((week) => (
          <div
            ...
            style={{
              "--calendar-all-day-lanes": Math.max(1, week.laneCount),
            } as CSSProperties}
          >
            <div className="calendar-month-week-cells">
              {week.days.map((day) => {
            ...
            {week.segments.length > 0 && (
              <div className="calendar-month-bars" aria-label="All-day events">
...
export function TimeGrid(props: {
  ...
  const allDayLayout = useMemo(
    () => weekAllDaySegments(props.events, rangeStart, props.days),
    [props.events, props.days, rangeStart.getTime()],
  );
  ...
  return (
    <div className="calendar-time-view">
      <div
        ...
        style={{ gridTemplateColumns: `72px repeat(${props.days}, minmax(0, 1fr))` }}
      >
        <div />
        {columns.map((day) => (
          <button
      ...
      <div
        ...
        style={{
          ...
          "--calendar-all-day-lanes": Math.max(1, allDayLayout.laneCount),
        } as CSSProperties}
      >
        <div className="calendar-all-day-label">All day</div>
        <div
          className="calendar-all-day-bars"
          style={{ gridTemplateColumns: `repeat(${props.days}, minmax(0, 1fr))` }}
        >
        ...
        >
          ...
          {columns.map((day) => {
            ...
            const dayEvents = props.events.filter(
              (event) => !event.all_day
                && eventDateKey(event, props.timezone) === key,
            );
            ...
            return (
              <div
                ...
                onDoubleClick={() => props.onSelectDay(day)}
              >
                {Array.from({ length: HOUR_END - HOUR_START + 1 }, (_, index) => (
                  <div
                    key={index}
                ...
                {dayEvents.map((event) => {
                  const startMinutes = eventMinutes(event.start_at, props.timezone);
                  const endMinutes = eventMinutes(event.end_at, props.timezone);
                  const top = Math.max(
                    0,
                    (startMinutes - HOUR_START * 60) / 60 * HOUR_HEIGHT,
                  );
                  ...
                  return (
                    <button
                      ...
                      style={{
                        top: `${top}px`,
                        height: `${height}px`,
                        "--calendar-event-color": event.calendar_color,
                      } as CSSProperties}
                      onClick={() => props.onSelectEvent(event)}
                    >
                      <strong>{event.title}</strong>
                      <span>
                        {formatTime(event.start_at, props.timezone)}
                        {" – "}
                        {formatTime(event.end_at, props.timezone)}
                      </span>
                    </button>
                  );
                })}
  ...
  const grouped = useMemo(() => {
    ...
    props.events.forEach((event) => {
      if (event.all_day && event.start_date && event.end_date_exclusive) {
        ...
        const end = parseDateKey(event.end_date_exclusive);
            ...
            {dayEvents.map((event) => (
              ...
              >
                <span
                  ...
                  style={{ background: event.calendar_color }}
                />
                <span className="calendar-agenda-time">
                  {eventTimeLabel(event, props.timezone)}
                </span>
