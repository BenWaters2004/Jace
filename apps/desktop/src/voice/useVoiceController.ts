...
// Streamed TTS tuning.
//
// The first chunk is deliberately allowed to be short so Jace starts speaking
// quickly. Every chunk after that is longer, because each extra split is one
// more synthesis request and one more opportunity for an audible seam.
const MIN_FIRST_CHUNK_CHARS = 12;
const MIN_STREAM_CHUNK_CHARS = 70;
const MAX_STREAM_BUFFER_CHARS = 400;

// How many chunks may be synthesising ahead of what is currently playing. This
// is the whole reason playback is continuous: chunk N+1 is being generated by
// Kokoro while chunk N is still audible, so its buffer is ready to schedule the
// instant the previous one ends.
const TTS_PREFETCH_AHEAD = 3;

// Small scheduling lead so the first source never starts in the past.
const PLAYBACK_LEAD_SECONDS = 0.03;

// Upper bound on how long speak() will await one utterance.
const SPEECH_WATCHDOG_MS = 120_000;
...
interface InFlightSynthesis {
  generation: number;
  task: Promise<SynthesisResult | null>;
}
 ...
 *
 * The previous boundary pattern was `/[.!?]+["')\\]]?(?=\\s|$)/g`. Inside a
 * regex literal `\\s` matches a literal backslash followed by "s", not
 * whitespace, so sentence ends were never detected mid-stream. Every chunk was
 * therefore produced by the emergency length splitter below, which cuts in the
 * middle of a sentence — the "half a sentence, then a pause" symptom.
 */
function pullSpeechChunks(
  ...
  // Constructed per call so lastIndex never leaks between invocations.
  const boundary = /[.!?…]+["'”’)\]]*(?=\s|$)/g;
  ...
  while ((match = boundary.exec(input)) !== null) {
    ...
    // Short fragments are held back so decimals, "Mr." and similar do not
    // become their own utterances.
    if (candidate.length >= minChars) {
      chunks.push(candidate);
      cursor = end;
      // Only the first chunk of a response is allowed to be short.
      minChars = Math.max(minChars, MIN_STREAM_CHUNK_CHARS);
    }
  }

  let remainder = input.slice(cursor);

  // Safety valve for a model that produces a very long unpunctuated clause.
...
export function useVoiceController(
  ...
  // Persistent playback graph. Creating an AudioContext per utterance cost tens
  // of milliseconds each time and made every chunk boundary audible.
  const playbackContextRef = useRef<AudioContext | null>(null);
  const playbackGainRef = useRef<GainNode | null>(null);
  const nextStartAtRef = useRef(0);
  const activeSourcesRef = useRef<Set<AudioBufferSourceNode>>(new Set());

  // Fallback path for runtimes without Web Audio.
  const fallbackAudioRef = useRef<HTMLAudioElement | null>(null);
  const fallbackUrlRef = useRef<string | null>(null);

  const speechAbortsRef = useRef<Set<AbortController>>(new Set());

  const meterContextRef = useRef<AudioContext | null>(null);
  const meterOwnsContextRef = useRef(false);
  ...
  const responseSpokeAnyRef = useRef(false);

  const speechGenerationRef = useRef(0);
  const pendingTextsRef = useRef<SpeechQueueItem[]>([]);
  const inFlightRef = useRef<InFlightSynthesis[]>([]);
  const pipelineRunningRef = useRef(false);
  const idleResolversRef = useRef<Array<() => void>>([]);
  const speechErrorRef = useRef<{ generation: number; error: Error } | null>(null);
  ...
  const stopAmplitudeMeter = useCallback(() => {
    ...
    const ownsContext = meterOwnsContextRef.current;
    meterContextRef.current = null;
    meterOwnsContextRef.current = false;

    // The TTS playback context is long-lived and shared, so it is never closed
    // here — only a context this meter created (the microphone one) is.
  ...
  const startAmplitudeMeter = useCallback(
    (
      ...
      meterOwnsContextRef.current = ownsContext;
    ...
    if (existing && existing.state !== "closed") {
      if (existing.state === "suspended") {
        void existing.resume().catch(() => {
          // Resume failures surface as silence, which the caller already
          // handles via the fallback element path.
        });
      }
      return existing;
    }
  ...
  const stopScheduledAudio = useCallback(() => {
    for (const source of activeSourcesRef.current) {
      try {
        ...
        // A source that already ended throws; that is fine.
      }
    }
  ...
  const maybeFinishSpeech = useCallback(() => {
    ...
    resolveSpeechIdle();
  }, [resolveSpeechIdle, setPhase, stopAmplitudeMeter]);

  const stopCurrentAudio = useCallback(() => {
    for (const controller of speechAbortsRef.current) {
      controller.abort();
    }
    speechAbortsRef.current.clear();
    ...
    resolveSpeechIdle();
  }, [resolveSpeechIdle, setPhase, stopAmplitudeMeter, stopScheduledAudio]);
  ...
  const stopSpeaking = useCallback(() => {
    ...
    responseSpokeAnyRef.current = false;
  ...
  /**
   * Fetch and decode one chunk. Decoding here rather than at playback time is
   * what allows the next chunk to be scheduled sample-accurately against the
   * previous one.
   */
  const synthesizeChunk = useCallback(
    async (text: string, generation: number): Promise<SynthesisResult | null> => {
      if (generation !== speechGenerationRef.current) return null;

      const controller = new AbortController();
      speechAbortsRef.current.add(controller);
      ...
      try {
        ...
        return { buffer, blob: null };
      } finally {
        speechAbortsRef.current.delete(controller);
      }
    },
    [getPlaybackContext],
  );
  ...
  const scheduleBuffer = useCallback(
    (buffer: AudioBuffer, generation: number) => {
      ...
      if (!playbackGainRef.current) {
        ...
        // Route playback through the analyser so JaceCore reacts to the real
        // waveform level of Kokoro's output.
        startAmplitudeMeter(context, gain, analyser, true, 4.4, false);
      }
      ...
      nextStartAtRef.current = startAt + buffer.duration;
      ...
      // An autoplay policy can leave the context suspended; retry so scheduled
      // buffers actually reach the speakers.
    ...
    [getPlaybackContext, maybeFinishSpeech, setPhase, startAmplitudeMeter],
  );

  /** Degraded path for runtimes without Web Audio. Gaps are unavoidable here. */
  const playBlobFallback = useCallback(
    async (blob: Blob, generation: number): Promise<void> => {
      if (generation !== speechGenerationRef.current) return;
  ...
  /**
   * Synthesis runs ahead of playback.
   *
   * Previously this queue awaited synthesis *and* playback for each chunk in
   * turn, so every chunk boundary contained a full Kokoro round trip of
   * silence. Now up to TTS_PREFETCH_AHEAD requests are in flight while audio is
   * playing, and finished buffers are scheduled end-to-end on one context.
   */
  const runSpeechPipeline = useCallback(async () => {
    if (pipelineRunningRef.current) return;
    ...
    try {
      while (mountedRef.current) {
        while (
          inFlightRef.current.length < TTS_PREFETCH_AHEAD &&
          pendingTextsRef.current.length > 0
        ) {
          const item = pendingTextsRef.current.shift();
          if (!item) break;
          if (item.generation !== speechGenerationRef.current) continue;

          const task = synthesizeChunk(item.text, item.generation);
          // stopSpeaking() can discard this entry before it is awaited. Marking
          // the promise handled avoids an unhandled rejection when its fetch is
          // aborted; the loop below still sees the rejection normally.
          task.catch(() => {});

          inFlightRef.current.push({ generation: item.generation, task });
        }

        const next = inFlightRef.current.shift();
        if (!next) break;

        if (next.generation !== speechGenerationRef.current) continue;
        ...
        try {
          ...
          if (result.buffer) {
            scheduleBuffer(result.buffer, next.generation);
          } else if (result.blob) {
            await playBlobFallback(result.blob, next.generation);
          }
        } catch (speechError) {
          if (
            next.generation !== speechGenerationRef.current ||
          ...
          const error =
            speechError instanceof Error
              ? speechError
              : new Error("Jace could not speak the response.");

          speechErrorRef.current = { generation: next.generation, error };
  ...
  const queueSpeechChunks = useCallback(
    (chunks: string[], generation: number) => {
      for (const chunk of chunks) {
        ...
        pendingTextsRef.current.push({ text: cleaned, generation });
      }
  ...
  const waitForSpeechIdle = useCallback(
    (generation: number): Promise<void> =>
      new Promise<void>((resolve) => {
        // Superseded speech, or a pipeline that already drained, resolves now.
        const alreadyDone =
        ...
        // If an audio context is suspended by an autoplay policy, `ended` never
        // fires. The watchdog keeps a caller such as the welcome line from
        // awaiting forever.
        const timer = window.setTimeout(finish, SPEECH_WATCHDOG_MS);

        idleResolversRef.current.push(finish);
      }),
    [],
  );
  ...
  const speak = useCallback(
    async (text: string): Promise<void> => {
      ...
      queueSpeechChunks([value], generation);
      await waitForSpeechIdle(generation);

      const failure = speechErrorRef.current;
      if (failure && failure.generation === generation) {
        speechErrorRef.current = null;
        throw failure.error;
      }
    },
    [queueSpeechChunks, stopSpeaking, waitForSpeechIdle],
  );
   ...
   * ingestResponseToken() begins queueing sentence-sized TTS chunks as soon as
   * they are complete.
   */
  ...
  const ingestResponseToken = useCallback(
    (token: string) => {
      ...
      if (chunks.length) {
        responseSpokeAnyRef.current = true;
    ...
    try {
      ...
      try {
        ...
        if (context) {
          ...
          // slightly stronger normalisation multiplier. This context is owned by
          // the meter and closed when recording stops.
          startAmplitudeMeter(context, source, analyser, false, 8.2, true);
        }
      } catch (meterError) {
  ...
  }, [refreshStatus, stopAmplitudeMeter, stopScheduledAudio]);
