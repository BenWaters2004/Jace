...
export default function App(
  ...
  useEffect(() => {
    if (connectionState === "backend-offline") return;
    if (!(settingsOpen && settingsSection === "permissions") && !controlStatus?.active_session) return;

    const timer = window.setInterval(() => {
      void refreshControl();
    }, 2500);
    return () => window.clearInterval(timer);
  }, [connectionState, settingsOpen, settingsSection, controlStatus?.active_session?.id, refreshControl]);

  // Phase 1D: approvals are global runtime work, not just chat-stream events.
  ...
  useEffect(() => {
    ...
    async function pollToolApprovals() {
      try {
        const response = await getPendingToolApprovals();
        if (cancelled) return;
        setPendingApproval((current) => {
          if (
            current &&
            response.approvals.some(
              (approval) => approval.approval_id === current.approval_id,
            )
          ) {
            // Preserve the foreground call_id when this approval came from the
            // streaming chat path; the REST representation intentionally does
            // not need one.
            return current;
          }
          return response.approvals[0] ?? null;
        });
      } catch {
        // Approval discovery is best-effort. The task remains persisted in
        // waiting_permission and will be rediscovered on the next successful poll.
      }
    }

    void pollToolApprovals();
    const timer = window.setInterval(() => {
      void pollToolApprovals();
    }, 750);
    return () => {
  ...
  async function submit(
    ...
    try {
      ...
      setPendingApproval((current) => current?.call_id ? null : current);
      isGeneratingRef.current = false;
      setIsGenerating(false);
    }
  }
  async function retryAssistant(assistantMessageId: string) {
    if (
    ...
    if (assistantIndex <= 0 || assistantIndex !== messages.length - 1) {
      setError("Try again is only available for the latest assistant response.");
      return;
    }
    ...
    if (
      ...
      user.role !== "user" ||
      assistant.model?.startsWith("agent:")
    ) {
      setError("This response cannot be regenerated from a normal user turn.");
      return;
    }

    const text = user.content.trim();
    const attachmentIds = (user.attachments ?? []).map(
    ...
    setIsGenerating(true);
    isGeneratingRef.current = true;
    voiceController.beginResponse(false);
    ...
    try {
      await sendChatStream(
        {
          ...
          voice_mode: false,
          retry_assistant_message_id: assistantMessageId,
          retry_user_message_id: user.id,
        },
        {
          onContext: (context) => {
            setMemoryContextCount(context.memory_count);
      ...
      await refreshConversations();
      const saved = await getConversation(activeConversationId);
      setMessages(mapMessages(saved));
      setSelectedModel(saved.model);
      setConversationPrompt(saved.system_prompt);
      await refreshTools();
      ...
      if (controller.signal.aborted) {
        ...
        setError(
          chatError instanceof Error
            ? chatError.message
            : "Jace could not regenerate the response.",
        );
      }
    } finally {
      abortRef.current = null;
    ...
    <>
      {screen === "chat" && (
        <ChatView
          ...
          onRetry={(assistantMessageId) => void retryAssistant(assistantMessageId)}
